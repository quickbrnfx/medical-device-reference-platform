# Stakeholder Analysis

## Objective

Identify and classify the stakeholders that influence, interact with, develop, verify, regulate, or are affected by the Version 1 reference platform.

This analysis establishes the foundation for user needs, system requirements, human factors activities, verification planning, and architectural decisions.

---

# Stakeholder Categories

| Category                 | Purpose                                                                                                                          |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| **Users**                | Individuals who directly interact with the device to accomplish its intended use.                                                |
| **Operators**            | Personnel responsible for configuring, operating, or monitoring the system in its intended environment.                          |
| **Patients**             | Individuals whose physiological data is measured or whose care is supported by the device.                                       |
| **Developers**           | Engineers responsible for designing, implementing, and maintaining the platform.                                                 |
| **Verification**         | Stakeholders responsible for demonstrating that the system satisfies its specified requirements through verification activities. |
| **Validation**           | Stakeholders responsible for confirming that the system fulfills user needs and intended use in its operational environment.     |
| **Manufacturing**        | Personnel responsible for assembly, production, calibration, and release.                                                        |
| **Service & Support**    | Personnel responsible for installation, maintenance, troubleshooting, and lifecycle support.                                     |
| **Regulatory & Quality** | Individuals or organizations responsible for compliance, quality management, and regulatory oversight.                           |


---

# Stakeholder Summary

| Stakeholder | Category | Primary Interest | Interaction | Influence |
| ----------- | -------- | ---------------- | ----------- | --------- |


---

# Observations

...

---

# Lessons Learned

...
